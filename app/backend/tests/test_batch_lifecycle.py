"""Batch rows record how the run ended, and the endpoint survives a failed write.

Salvaged from the Phase 2.4 verification run, logged as T-007. A batch row is
written before processing starts, so without a closing update a finished batch is
indistinguishable in History from one that died halfway.
"""
...
def test_completed_batch_is_marked_with_its_counts(client, app_module):
    """A row left reading processing after the response has gone out means the
...
def test_skipped_images_are_counted_separately(client, app_module):
    """processed plus skipped should equal what the caller sent, or the History
    row misrepresents the batch."""
    entries = [
        ("good.png", png_bytes()),
...
def test_collapsed_run_is_marked_failed(client, app_module, monkeypatch):
    """process_batch traps per-file errors, so an exception escaping it means
    the whole run collapsed and no image got a verdict."""
...
def test_results_survive_a_failed_batch_insert(client, app_module):
    """The verdicts are what the caller asked for and the row is bookkeeping, so
    a database failure is reported alongside the results, not instead of them."""
...
def test_no_status_update_for_a_row_that_was_never_inserted(client, app_module):
    """The per-image rows carry batch_id as a foreign key, so if the batch row
    failed there is nothing to update and nothing that could be written."""
    app_module.db = FakeDb(fail_save_batch=True)
...
def test_batch_stores_p_ai_not_p_real(client, app_module):
    """analyze writes P(AI) into the same two columns, so writing P(real) here
    would put both meanings in one column with nothing to tell them apart.
    This is the inversion that has already caused one live bug."""
    post_batch(client)
