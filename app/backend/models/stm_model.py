...
# Handcrafted feature extractor combining texture, frequency, colour, and noise cues
class STMFeatureExtractor:
    def __init__(self, image_size: int = 256):
        self.image_size = image_size

    # Concatenate all feature groups into a single flat vector
    ...
    # Gradient orientation histogram, capturing edge and shape structure
    ...
    # Local binary patterns at two radii, capturing fine texture
    ...
    # DCT energy statistics in low, mid, and high frequency zones
    ...
    # Gaussian-subtracted noise residuals, where AI images show distinctive patterns
...
# Wraps STMFeatureExtractor + LightGBM for use in the nn.Module evaluation pipeline
class STMDetector(nn.Module):
    ...
    # Undo ImageNet normalisation and convert batch to uint8 HWC numpy arrays
    ...
    # Exposed separately because per-prediction feature contributions need the
    # same vector the classifier scored, and forward discards it
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        # Convert probabilities to logits to match the nn.Module interface
