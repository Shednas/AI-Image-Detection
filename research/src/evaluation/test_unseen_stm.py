...
def run_inference(model, paths, label, quality, match_training=False):
    probs, labels, names = [], [], []
    for p in paths:
        try:
...
def evaluate(model, real_paths, ai_paths, quality, match_training=False):
    rp, rl, rn = run_inference(model, real_paths, label=1, quality=quality,
                               match_training=match_training)
    ap, al, an = run_inference(model, ai_paths, label=0, quality=quality,
                               match_training=match_training)
    all_p = rp + ap
    all_l = rl + al
...
if __name__ == "__main__":
    ...
    results_dir = Path(args.results_dir or f"results/stm/stage_{args.stage}/unseen")
    ...
    for ds in datasets:
        ...
        for deg in degradations:
            ...
            m, records = evaluate(model, real_paths, ai_paths, JPEG_QUAL[deg],
        ...
        if args.dump_probabilities:
            dump = {"dataset": ds, "stage": args.stage, "model": "stm",
                    "match_training_preprocessing": args.match_training_preprocessing,
