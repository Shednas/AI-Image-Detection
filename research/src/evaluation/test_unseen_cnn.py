...
# Compute metrics over combined real+AI paths; handle AI-only datasets separately
def evaluate(model, real_paths, ai_paths, quality, device, match_training=False):
    ...
    if not all_p:
        return None, records
    # AI-only dataset: report detection rate (fraction correctly flagged as AI)
    if not rp:
        det_rate = sum(1 for p in ap if p < 0.5) / len(ap)
        print(f"    DetectionRate: {det_rate:.4f}  (AI-only, no real images)")
        return {"detection_rate": det_rate, "n_ai": len(ap),
                "note": "AI-only dataset; no real images - detection rate only"}, records
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Test CNN on unseen datasets")
    ...
    ckpt = Path(f"checkpoints/cnn/stage_{args.stage}/best_cnn.pt")

    model = CNNDetector().to(device)
    model.load_state_dict(torch.load(ckpt, map_location=device, weights_only=True))
    model.eval()
    print(f"Loaded CNN Stage {args.stage} from {ckpt}")

    results_dir = Path(args.results_dir or f"results/cnn/stage_{args.stage}/unseen")
    ...
    for ds in datasets:
        ...
        out = {"dataset": ds, "stage": args.stage, "model": "cnn",
        ...
        if args.dump_probabilities:
            dump = {"dataset": ds, "stage": args.stage, "model": "cnn",
                    "match_training_preprocessing": args.match_training_preprocessing,
